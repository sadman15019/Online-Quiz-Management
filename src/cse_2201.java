 ...
 * @author Sadman
 */
public class cse_2201 extends javax.swing.JFrame {

    /**
     * Creates new form cse_2201
     */
    String student_id;
    String questionid="1";
    String ans;
    ...
    public void timer(){
    state = true;
        Thread t = new Thread(){//for instantiating timer class which means call the constructor and creating object for that class 
          public void run(){
              for(;;){
                  if(state == true){
                      try{
        ...
        public cse_2201(String a) {
        initComponents();
        this.setLocationRelativeTo(null);
        student_id=a;
        String course="Cse 2201";
        try
        {
             database_connection dc=new database_connection();
        ...
        try{
            database_connection dc=new database_connection();
            ResultSet rs=dc.state.executeQuery("SELECT COUNT(*) AS rowcount FROM cse_2201");
            rs.next();
            c=rs.getString("rowcount");
            int d=Integer.valueOf(c);
            d++;
            c=String.valueOf(d);
            rs=dc.state.executeQuery("select * from cse_2201 where question_no='"+questionid+"'");
            while(rs.next())
            {
        ...
        public void question()
        {
            try{
            database_connection dc=new database_connection();
            ResultSet rs=dc.state.executeQuery("select * from cse_2201 where question_no='"+questionid+"'");
            while(rs.next())
            {
                  idlabel.setText(""+questionid);
                  questionlabel.setText(rs.getString(2));
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(cse_2201.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
